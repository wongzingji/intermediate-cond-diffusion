...
def main():
    ...
    print("creating VitMem...")

    # base image
    NORMALIZE_MEAN = (0.0, 0.0, 0.0)
    NORMALIZE_STD = (1.0, 1.0, 1.0)
    transform_vitmem = transforms.Compose([
    ...
    def cond_fn(operation, x, t, out, y=None):
        ...
        image_in = transform_vitmem(out['pred_xstart'])
        pred_scores = vitmem(image_in) # TODO: batch_size is imposed to be 1
    ...
    def create_samples(cond_fn, batch, names, skip_timesteps, operation, seed, mem_scores):
        ...
        for i in range(pred_xstart.size(0)):
            saved_name = f"skip{skip_timesteps}_{operation}_scale{args.mem_scale}_seed{seed}_vitmem.png"
            utils.save_image(
                        pred_xstart[i, :, :, :],
